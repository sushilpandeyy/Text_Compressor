...
//global variables
int number_nodes = 0, qend = 1; 	     	//global variables for keep track of no.of nodes and end of the que
struct node_t pool[256] = {{0}};        //pool of nodes
node qqq[255], *q = qqq-1;      	//queue
...
//function ued to insert a node into the que
void qinsert(node n)
{
	int j, i = qend++;
	while ((j = i / 2)) {
...
void encode(FILE* fp_in, FILE* fp_out, unsigned int *freq ){
	...
	fprintf(fp_out,"%04d\n",lim);     //Saving the lenght of total code at the start of file.
	printf("\nEncoded:\n");
	for(i=0; i<lim; i++){
		if(temp[j] == '\0'){
			in = fgetc(fp_in);
